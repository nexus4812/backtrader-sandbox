...
def optimize_and_evaluate(symbol, start, end):
    ...
    # 結果を保存するディレクトリの作成
    os.makedirs(f'backtest_results/{symbol}/strategy/donchian-channel-opt', exist_ok=True)
    ...
    for i, run in enumerate(optimized_runs):
        for j, strategy in enumerate(run):
            ...
            fig.savefig(f'backtest_results/{symbol}/strategy/donchian-channel-opt/result_{i}_{j}_upper{upper_period}_lower{lower_period}.png')
            plt.close(fig)

            final_results_list.append((upper_period, lower_period, final_value, sqn))

    # 最適化結果をソートして表示
    sorted_by_sqn = sorted(final_results_list, key=lambda x: x[3]['sqn'], reverse=True)
    best_result = sorted_by_sqn[0]
    print(f"Best result for {symbol} - Upper Period: {best_result[0]}, Lower Period: {best_result[1]}, Final Portfolio Value: {best_result[2]:.2f}, SQN: {best_result[3]['sqn']:.2f}")
...
for symbol in symbols:
    best_results[symbol] = optimize_and_evaluate(symbol, start_date, end_date)

print("All best results:")
for symbol, result in best_results.items():
    print(f"{symbol} - Upper Period: {result[0]}, Lower Period: {result[1]}, Final Portfolio Value: {result[2]:.2f}, SQN: {result[3]['sqn']:.2f}")
